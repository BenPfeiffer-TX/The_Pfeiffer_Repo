...
= analyzer2(tiff_stack,newB,heightmap,T)
...
disp(t)
    for v = 1 : length(newB(:,t))
        if isempty(newB{v,t}) == false
            ...
            %temporarily stores pixel coords to do length calcs with
            ...
            rx = min(rowcol(:,1)):max(rowcol(:,1));
            ry = min(rowcol(:,2)):max(rowcol(:,2));
            region=heightmap(rx,ry,t);
            C=contourc(region,15);
            levels=find(C(1,:)<1); %Finds the index for the levels of C
